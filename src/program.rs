...
impl RunningProgram {
    ...
    //fn find_pc(&self) -> String {
      //  return self.program.get_pc();
      //  }
    //fn get_reg(&self) -> String {
      //  return self.program.get_pc_reg();
       // }
    pub fn get_drawing_tool(&self, pipeline:bool) -> Drawingtool {
        if !pipeline
        {
            let comp=self.parse_file_non_pipeline();
            return Drawingtool::new(comp);
        }
        else 
        {
            let comp=self.parse_file_non_pipeline();
            let mut draw_tool:Drawingtool= Drawingtool::new(comp);
            let comp2=self.parse_file_pipeline();
            draw_tool.set_pipeline_regs(comp2.6);
            return draw_tool
        }
    ...
    fn get_icode(&self,wtmv: Vec<WiretagMarker>) -> String {
        for wtm in wtmv.clone()
        {
            if wtm.get_from()==String::from("i10bytes")&& wtm.get_low()== 4 && wtm.get_high()==8
            {
                return wtm.get_to().clone()
            }

        }
        return String::from("null")
    }
    fn get_reg_out_signal_by_input(&self,input:String, registers:Vec<RegisterBank>) -> (String,String) { //reg_name, reg_output
        for reg in registers.clone()
        {
            let mut temp=reg.get_signal();
            for sig in temp.clone()
            {
                if sig.0==input
                {
                    return (reg.get_label().clone(),sig.1)
                }
            }
        }
        return (String::from("null"),String::from("null"))
    }
    pub fn get_pipeline_reg(&self,wtmv: Vec<WiretagMarker>, wires:Vec<Wireassign>, registers:Vec<RegisterBank>) -> Vec<String>
    {
        let mut pipeline_vec= Vec::new();
        let mut last_reg_input=self.get_icode(wtmv);
        let mut last_reg_info=self.get_reg_out_signal_by_input(last_reg_input.clone(),registers.clone());
        let mut last_reg_output = last_reg_info.1;
        let mut last_reg_name = last_reg_info.0;
        pipeline_vec.push(last_reg_name);
        let mut count = 1;
        let mut one_found = true;


        while count != 5 && one_found
        {
            one_found = false;
            for wire in wires.clone()
            {
                if wire.right.len()==1 
                {
                    if wire.right[0]== last_reg_output
                    {
                        last_reg_input = wire.left;
                        last_reg_info=self.get_reg_out_signal_by_input(last_reg_input.clone(),registers.clone());
                        last_reg_name = last_reg_info.0;
                        last_reg_output = last_reg_info.1;
                        pipeline_vec.push(last_reg_name);
                        one_found = true;
                        count = count + 1;
                    }
    ...
    pub fn parse_file_pipeline(&self) -> (String,String,Vec<WiretagMarker>, Vec<Wireassign>,Vec<FakeMux>, Vec<RegisterBank>, Vec<String>) {
        let full_pc:(String,String)=self.get_full_pc();
        let parse_i10:Vec<WiretagMarker>=self.program.label_wires();
        let wire_assignments: Vec<Wireassign>=self.get_all_wire_assignments();
        let muxes:Vec<FakeMux>=self.get_all_muxes();
        let registers:Vec<RegisterBank>=self.get_all_reg();
        let pipeline_reg_vec= self.get_pipeline_reg(parse_i10.clone(),wire_assignments.clone(),registers.clone());

        return(full_pc.0,full_pc.1,parse_i10, wire_assignments, muxes,registers, pipeline_reg_vec)
    }
